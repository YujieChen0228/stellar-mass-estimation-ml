...
for i in range(len(redshift_bins) - 1):
    ...
    try:
        ...
        print(
            f"红移区间 {z_low:.1f}-{z_high:.1f}: 样本量={len(selected)}, 比值={ratio:.3f}, R²={r2_val:.3f}, RMSE={rmse:.3f}")

    except (IndexError, ValueError) as e:
        print(f"红移区间 {z_low:.1f}-{z_high:.1f} 计算特征重要性时出错: {e}")
        continue
...
if not results_df.empty:
    ...
    # 绘制比值线
    plt.plot(results_df['z_center'], results_df['ratio'], 'o-',
             linewidth=2, markersize=8, label='Ratio')
    ...
    plt.tight_layout()
    plt.savefig('feature_importance_ratio_evolution_single_run.png', dpi=300)
    plt.show()

    # 创建第二个图表显示模型性能
    plt.figure(figsize=(10, 6))
    plt.plot(results_df['z_center'], results_df['r2_score'], 's-',
             linewidth=2, markersize=8, color='green', label='R² Score')
    plt.xlabel('Redshift (z)', fontsize=14)
    plt.ylabel('R² Score', fontsize=14)
    plt.title(f"Model Performance Across Redshift Bins\n(Single Run), bin range={bin_width}", fontsize=16)
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig('model_performance_r2_single_run.png', dpi=300)
    plt.show()

    # 创建第三个图表显示RMSE
    plt.figure(figsize=(10, 6))
    plt.plot(results_df['z_center'], results_df['rmse'], 's-',
             linewidth=2, markersize=8, color='red', label='RMSE')
    plt.xlabel('Redshift (z)', fontsize=14)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('model_performance_rmse_single_run.png', dpi=300)
    plt.show()
